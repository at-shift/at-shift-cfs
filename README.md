...
Current maintenance version: 3.0.2.3 (現在のメンテナンスバージョン: 3.0.2.3)
...
- 投稿タイトル設定、フロントエンドのビジュアルエディタフォント、Extras の表示設定、タブグループの編集画面UIを改善しました。
...
- 保存・公開フィールドで、ステータス変更の選択だけでは表示中のステータスを変えず、保存時に反映するよう調整しました。
- 保存・公開フィールドの公開・予約投稿まわりの表示文言を整理しました。
- 保存・公開フィールドで、下書きとレビュー待ちの投稿にもプレビューボタンを表示するようにしました。
- 自動保存時に、保存・公開フィールドのステータス変更が反映されないようにしました。
- 保存・公開フィールドのごみ箱リンクを WordPress 標準に合わせて左寄せに戻しました。
- 保存・公開フィールドの公開範囲設定で、パスワード入力欄の表示位置を修正しました。
- 同梱している翻訳ファイルを更新しました。
...
- 条件分岐グループで、条件の選択を必須にできるオプションを追加しました。
- アコーディオングループ内に必須フィールドがある場合、見出しに分かるよう表示を追加しました。
- 投稿カテゴリーで、検索ボックスと選択済みフィルターを非表示にできるオプションを追加しました。
- 投稿カテゴリーで、デフォルトカテゴリーの扱いと親子カテゴリーの自動選択が衝突する問題を修正しました。
- 保存・公開フィールドのレイアウトと表示デザインを調整しました。
- 同梱している翻訳ファイルを更新しました。
