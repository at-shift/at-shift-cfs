...
- 公開権限のないユーザーが公開済み・予約投稿・非公開の投稿を更新した際に、意図せずレビュー待ちへ変更されないよう修正しました。
- 投稿タイトル（標準）フィールドの更新時に、WordPress 側で明示的に変更されていない既存スラッグを保持するよう修正しました。
...
- フィールド見出しのスタイルを調整し、ユーザーの管理画面カラースキームに合わせながら見出しとして識別しやすくしました。
- ループ行の余白、カラースキームに合わせた行スタイル、行操作UIを調整しました。
- ループ行の見出し表示を改善し、動的な行ラベル指定の説明を追加しました。
