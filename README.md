...
Current maintenance version: 3.0.2.4 (現在のメンテナンスバージョン: 3.0.2.4)
...
- 一部のフィールド数が多いサイトにおいて、フィールド値や配置ルールが保存できない問題に対する警告・改善表示を実装しました。
- カテゴリー（標準・グローバル）フィールドに、カテゴリーを追加するオプションを追加しました。
- その他軽微な修正。
