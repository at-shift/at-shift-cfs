...
class Atshift_CFS_phone extends Atshift_CFS_field
{

    function __construct() {
        $this->name = 'phone';
        $this->label = __( 'Phone Number', 'atshift-fields-maintenance-for-custom-field-suite' );
    }


    function html( $field ) {
    ?>
        <input type="tel" name="<?php echo esc_attr( $field->input_name ); ?>" class="<?php echo esc_attr( $field->input_class ); ?>" value="<?php echo esc_attr( $field->value ); ?>" placeholder="<?php echo esc_attr( $this->get_input_placeholder( $field ) ); ?>" />
    <?php
    }
